window.onload=function(){
    ...
    function QfDate(){
      ...
      this.init=function(){
        $('body').append(
            ...
            '<div class="qfdateTitle_title" style="display: inline-block;line-height:35px;padding: 0 20px;">选择时间</div>'+
            '<div class="qfdateOkBtn" onclick="gettime()" style="display: inline-block;line-height:35px;padding: 0 20px;float: right; ">确定</div>'+
            '</div>'+
             '<div class="qfscroll qfdateYearScroll"></div>'+
            '<div class="qfscroll qfdateMonthScroll"></div>'+
            '<div class="qfscroll qfdateDayScroll"></div>'+
            '<div class="qfscroll qfdateHoursScroll"></div>'+
            // '<div class="qfscroll qfdateMinutesScroll"></div>'+
            '</div>'+
            '</div>'
            );
            ...
            qfdateHoursScroll=new QfScroll($('.qfdateHoursScroll'));
            // qfdateMinutesScroll=new QfScroll($('.qfdateMinutesScroll'));
           qfdateYearScroll.init();
          qfdateMonthScroll.init();
          qfdateDayScroll.init();
          qfdateHoursScroll.init();
          // qfdateMinutesScroll.init();
          ...
          qfdateHoursScroll.setTodo(function(){
              _this.checkDays();
          });
          // qfdateMinutesScroll.setTodo(function(){
          //     _this.checkDays();
          // });
      ...
      this.getDate=function(){
          return _this.getY()+'/'+_this.getM()+'/'+_this.getD()+' '+_this.getH()+':00';
      };
       this.getY=function(){
      ...
      this.getH=function(){
          return qfdateHoursScroll.getVal();
      };
      // this.getMi=function(){
      //     return qfdateMinutesScroll.getVal();
      // };
      ...
      this.showView=function(){
          var now=new Date();
            ...
            //   nowMi="0"+nowMi;
            // }
            for (var i = 0; i < nowY-startYear+50; i++) {
                yData[i]=startYear+i;
            };
            qfdateYearScroll.setData(yData,nowY-startYear);
            for (var i = 0; i < 12; i++) {
                 mData[i]=i+1;
              if(mData[i]>=1&&mData[i]<=9){
              mData[i]="0"+mData[i];
             }
             };
            ...
            for (var i = 0; i < 24; i++) {
                hData[i]=i;
              if(hData[i]>=1&&hData[i]<=9){
              hData[i]="0"+hData[i];
             }
             };
            qfdateHoursScroll.setData(hData,nowH);
            ...
            //     miData[i]=i;
            // };
            // qfdateMinutesScroll.setData(miData,nowMi);
          ...
          if (isdate(inputValue)) {
                ...
                qfdateHoursScroll.setData(hData,dateValues[1]-1);
                // qfdateMinutesScroll.setData(miData,dateValues[1]-1);
          }
            qfdateMain.show();
      };
    };
    ...
    function isdate(str){
        ...
        return (d.getFullYear()==result[1] && d.getMonth()+1==result[3] && d.getDate()==result[4]&&d.getHours()==result[5]);
    };
