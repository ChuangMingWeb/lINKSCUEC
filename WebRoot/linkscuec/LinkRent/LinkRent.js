...
function tosearch() {
    ...
    $.ajax({
        ...
        url: 'http://localhost:8080/Link/StatusOpinion', //测试接口
             ...
             if(regStatus){
                 if(chuzuStatus){
                    if(zuchuStatus){
                        ...
                        $.ajax({
                            type:'post',
                            url:'http://localhost:8080/Link/ModifyQuery', //测试接口  实际接口RentQuery.java
                            success:function(result){     
                                var result = JSON.parse(result);
...
$(document).ready(function () {
    ...
    if (status == 1) {
        ...
        $.ajax({
            type: 'post',
            url: 'http://localhost:8080/Link/RentQuery', //测试接口
            success: function (result) {
                console.log(result);
                var result = JSON.parse(result);
                startTime = result.startTime;
